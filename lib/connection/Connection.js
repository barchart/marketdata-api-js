	...
	function ConnectionInternal(marketState, instance) {
		...
		function broadcastEvent(subscriptionType, message) {
			...
			if (subscriptionType === 'events') {
				...
				__logger.warn(`Broadcast [ ${__instance} ]: Unable to notify subscribers of [ ${subscriptionType} ] event.`);
			...
			if (listeners) {
				listeners.forEach((listener) => {
					try {
						...
						__logger.warn(`Broadcast [ ${__instance} ]:: A consumer-supplied listener for [ ${subscriptionType} ] events threw an error. Continuing.`, e);
					}
				});
			}
		}
		...
		function processSnapshots(symbols) {
			...
			retrieveSnapshots(symbols, __loginInfo.username, __loginInfo.password)
				.then((quotes) => {
					if (__connectionState !== state.authenticated) {
						return;
					}
