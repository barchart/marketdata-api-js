...
var convertConsumerSymbolToProducerSymbol = require('./../../../util/convertConsumerSymbolToProducerSymbol');
	...
	var Connection = function() {
		...
		function off() {
			...
			var unsubscribe = function(trackingMap, taskName, listenerMap, additionalTrackingMaps, additionalListenerMaps) {
				...
				var producerSymbol = convertConsumerSymbolToProducerSymbol(consumerSymbol);
				...
				if (previousListeners.length > 0 && currentListeners.length === 0) {
					...
					if (consumerSymbol !== producerSymbol) {
						...
						for (var i = 0; i < additionalListenerMaps.length; i++) {
							if (additionalListenerMaps[i].hasOwnProperty(consumerSymbol)) {
								remove = false;
								break;
							}
						}
			...
			switch (eventId) {
				...
				case 'marketUpdate': {
					...
					unsubscribe(__marketUpdateSymbols, "MD_STOP", __listeners.marketUpdate, [ __cumulativeVolumeSymbols ], [ __listeners.cumulativeVolume ]);
				...
				case 'cumulativeVolume': {
					...
					unsubscribe(__cumulativeVolumeSymbols, "MD_STOP", __listeners.cumulativeVolume, [ __marketUpdateSymbols ], [ __listeners.marketUpdate ]);
		...
		function on() {
			...
			var subscribe = function(trackingMap, taskName, listenerMap, additionalTrackingMaps) {
				...
				if (consumerSymbol !== producerSymbol) {
					...
					if (add) {
						consumerSymbols = consumerSymbols.slice(0);
						consumerSymbols = consumerSymbols.push(consumerSymbol);
