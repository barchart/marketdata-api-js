...
module.exports = (() => {
	...
	function parsePrice(value, unitCode, fractionSeparator, specialFractions, thousandsSeparator) {
		...
		if (unitCodeItem.supportsFractions && fractionSeparator !== '.') {
			const digitGroups = value.split(fractionSeparator);

			if (digitGroups.length !== 2) {
				return Number.NaN;
			}

			const integerPart = parseInt(digitGroups[0]);
			const fractionPart = parseInt(digitGroups[1]);

			if (is.nan(integerPart) || is.nan(fractionPart)) {
				return Number.NaN;
			}

			const denominator = unitCodeItem.getFractionFactor(is.boolean(specialFractions) && specialFractions);
