...
module.exports = (() => {
	...
	function formatPrice(value, unitCode, fractionSeparator, specialFractions, thousandsSeparator, useParenthesis) {
		...
		const unitCodeEnum = UnitCode.parse(unitCode);
		...
		if (!unitCodeEnum.supportsFractions || fractionSeparator === '.') {
			return formatDecimal(value, unitCodeEnum.decimals, thousandsSeparator, useParenthesis);
		} else {
