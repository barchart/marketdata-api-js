...
var BarchartFormatter = function(fractionSeparator, specialFractions) {
                        ...
                        case 'BOOK': {
                            ...
                            if ((node.attributes.getNamedItem('askprices')) && (node.attributes.getNamedItem('asksizes'))) {
                                ...
                                for (var i = 0; i < ary1.length; i++) {
                                    message.asks.push({ "price" : BarchartMessageParser.ParseValue(ary1[i], message.unitcode), "size" : parseInt(ary2[i])});
                                }
                            }
                            ...
                            if ((node.attributes.getNamedItem('bidprices')) && (node.attributes.getNamedItem('bidsizes'))) {
                                ...
                                for (var i = 0; i < ary1.length; i++) {
                                    message.bids.push({ "price" : BarchartMessageParser.ParseValue(ary1[i], message.unitcode), "size" : parseInt(ary2[i])});
                                }
                            }
                        ...
                        case 'QUOTE': {
                            for (var i = 0; i < node.attributes.length; i++) {
                                switch (node.attributes[i].name) {
                                    ...
                                    case 'bid':
                                        message.bidPrice = BarchartMessageParser.ParseValue(node.attributes[i].value, message.unitcode);
                                        break;
                                    case 'bidsize':
                                        message.bidSize = parseInt(node.attributes[i].value);
                                        break;
                                    case 'ask':
                                        message.askPrice = BarchartMessageParser.ParseValue(node.attributes[i].value, message.unitcode);
                                        break;
                                    case 'asksize':
                                ...
                                for (var j = 0; j < node.childNodes.length; j++) {
                                    if (node.childNodes[j].nodeName == 'SESSION') {
                                        ...
                                        if (attributes.getNamedItem('last'))
                                            session.lastPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('last').value, message.unitcode);
                                        if (attributes.getNamedItem('previous'))
                                            session.previousPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('previous').value, message.unitcode);
                                        if (attributes.getNamedItem('open'))
                                            session.openPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('open').value, message.unitcode);
                                        if (attributes.getNamedItem('high'))
                                            session.highPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('high').value, message.unitcode);
                                        if (attributes.getNamedItem('low'))
                                            session.lowPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('low').value, message.unitcode);
                                        ...
                                        if (attributes.getNamedItem('settlement'))
                                            session.settlementPrice = BarchartMessageParser.ParseValue(attributes.getNamedItem('settlement').value, message.unitcode);
                    ...
                    case '2': {
                        ...
                        switch (message.subrecord) {
                            case '0': {
                                ...
                                message.value = BarchartMessageParser.ParseValue(msg.substring(pos + 7, pos2), message.unitcode);
                                ...
                                message.time = BarchartMessageParser.ParseTimestamp(msg.substr(msg.indexOf('\x03') + 1, 9));
                                break;
                            }
                            case '1':
                            case '2':
                            ...
                            case '4': {
                                ...
                                message.openPrice = BarchartMessageParser.ParseValue(ary[0], message.unitcode);
                                message.highPrice = BarchartMessageParser.ParseValue(ary[1], message.unitcode);
                                message.lowPrice = BarchartMessageParser.ParseValue(ary[2], message.unitcode);
                                message.lastPrice = BarchartMessageParser.ParseValue(ary[3], message.unitcode);
                                message.bidPrice = BarchartMessageParser.ParseValue(ary[4], message.unitcode);
                                message.askPrice = BarchartMessageParser.ParseValue(ary[5], message.unitcode);
                                message.previousPrice = BarchartMessageParser.ParseValue(ary[7], message.unitcode);
                                message.settlementPrice = BarchartMessageParser.ParseValue(ary[10], message.unitcode);
                            ...
                            case '7': {
                                var pos2 = msg.indexOf(',', pos + 7);
                                message.tradePrice = BarchartMessageParser.ParseValue(msg.substring(pos + 7, pos2), message.unitcode);
                            ...
                            case '8': {
                                var pos2 = msg.indexOf(',', pos + 7);
                                message.bidPrice = BarchartMessageParser.ParseValue(msg.substring(pos + 7, pos2), message.unitcode);
                                pos = pos2 + 1;
                                pos2 = msg.indexOf(',', pos);
                                message.bidSize = parseInt(msg.substring(pos, pos2));
                                pos = pos2 + 1;
                                pos2 = msg.indexOf(',', pos);
                                message.askPrice = BarchartMessageParser.ParseValue(msg.substring(pos, pos2), message.unitcode);
                                pos = pos2 + 1;
                                pos2 = msg.indexOf(',', pos);
                        ...
                        switch (message.subrecord) {
                            case 'B': {
                                ...
                                for (var i = 0; i < ary.length; i++) {
                                    ...
                                    if (c <= 'J')
                                        message.asks.push({"price" : BarchartMessageParser.ParseValue(ary2[0], message.unitcode), "size" : parseInt(ary2[1])});
                                    else
                                        message.bids.push({"price" : BarchartMessageParser.ParseValue(ary2[0], message.unitcode), "size" : parseInt(ary2[1])});
                                }
...
var BarchartMarketState = function() {
    ...
    var _processMessage = function(message) {
        ...
        if ((!q.day) && (message.day)) {
            q.day = message.day;
            q.dayNum = BarchartUtil.DayCodeToNumber(q.day); 
        }

        if (message.type != 'BOOK') {
            if ((q.day) && (message.day)) {

                var dayNum = BarchartUtil.DayCodeToNumber(message.day);
        ...
        switch (message.type) {
            ...
            case 'REFRESH_QUOTE': {                
                ...
                var info = BarchartUtil.ParseSymbolType(p.symbol);
                if (info) {
                    if (info.type == 'future') {
                        p.root = info.root;
                        p.month = info.month;
...
var BarchartRealtimeData = function() {
    /* Constants */
    _API_VERSION = 4;

    var __profileService = new BarchartProfiles();
    ...
    var __feedMessages = [];
    var __marketState = new BarchartMarketState();
    ...
    var broadcastEvent = function(eventId, message) {
        var ary;
        switch (eventId) {
            case 'disconnect':
    ...
    var onNewMessage = function(msg) {
        var message;
        try {
            message = BarchartMessageParser.Parse(msg);
            if (message.type) {
    ...
    var processFeedMessages = function() {
        var done = false;
        var suffixLength = 9;
    ...
    var handleNetworkMessage = function(msg) {
    ...
    var __function_connect = function(server, username, password) {
        if (__connection)
            return;
    ...
    var __function_disconnect = function() {
        __state = 'DISCONNECTED';
    ...
    var __function_requestSymbols = function(symbols) {
    ...
    return {
        connect : __function_connect,
        disconnect : __function_disconnect,
...
var BarchartProfiles = function() {
    var _profiles = {};

    var _getProfile = function(symbol) {
        return _profiles[symbol];
    };

    var _getProfiles = function(symbols, callback) {
        $.ajax({
...
var BarchartHistoricalData = function() {
    var _url = 'proxies/historicaldata';

    var _getHistoricalData = function(params, callback) {
        $.ajax({
            url : _url,
            dataType : 'jsonp',
            data : params
